            ...
            <c:forEach var="item" items="${list}" varStatus="num">
                <li>
                    <a class="to-detail" href="javascript:">
                        ...
                        <h3 class="name">总价：&yen;${item.ptotal}</h3><br/>
                        <h4 class="name">
                            状态：
                        ...
                        <div class="down-btns btns2">
                            <p class="desc">数量：
                                <input type="text" value="${item.tpcount}" style="width:60px;" readonly />
                            </p>
                        </div>
                    </a>
                </li>
    ...
    <div>
        <c:choose>
            ...
            <c:when test="${totalPage == currPageNumber}">
                <a class="prev" href="${cyw }/tup/orderList?currentPageNumber=${currPageNumber-1}">上一页</a>
            </c:when>
            <c:when test="${currPageNumber<totalPage && currPageNumber>1}">
                <a class="prev" href="${cyw }/tup/orderList?currentPageNumber=1">首页</a>
                <a class="prev" href="${cyw }/tup/orderList?currentPageNumber=${currPageNumber-1}">上一页</a>
                <a class="prev" href="${cyw }/tup/orderList?currentPageNumber=${currPageNumber+1}">下一页</a>
                <a class="prev" href="${cyw }/tup/orderList?currentPageNumber=${totalPage}">尾页</a>
            </c:when>
            <c:otherwise>
                <a class="prev" href="${cyw }/tup/orderList?currentPageNumber=${currPageNumber+1}">下一页</a>
            </c:otherwise>
